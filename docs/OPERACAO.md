# Manual de operação do sofia-eval

Para o que este repositório é e por que existe, comece pelo
[README](../README.md). Isto aqui é o manual: instalação, isolamento,
vocabulário de verificação, formato dos cenários e as armadilhas.
...
3. **Chave da OpenRouter: a de teste, com teto próprio.** Ela vive no `.env` do
...
Ele próprio é verificado por mutação, e a prática se repete a cada chave nova.
Na mais recente (07/09), quebrando de propósito a derivação do
`phone_number_id`, o autoteste passou de 41/41 para 39/41 e acusou exatamente os
dois casos certos — enquanto o caso do determinismo continuou verde, que é a
discriminação correta, porque uma constante É determinística. Antes disso, o
mesmo exercício com o `<=` do teto de respostas e a comparação de status do
`agendamento_status` levou o autoteste de 10/10 para 8/10. Teste que só foi visto passar não provou que consegue reprovar — a
...
`google/gemini-3.7-flash`. **Os 16 cenários foram recalibrados em 2026-09-07**,
contra o `sofia-bot` que entrou em produção nesse dia, todos sob o mesmo modelo
e com `phone_number_id` por cenário — é a primeira vez que a suíte inteira tem
teto medido contra o mesmo prompt e o mesmo commit. A data, o modelo, o commit
e os três números de cada medição estão no comentário ao lado do teto
correspondente, no YAML — é lá que se confere de onde veio, não aqui.

**A folga precisa ser remedida de vez em quando, e não só quando um cenário
reprova.** A recalibração de 07/09 começou por uma medição de rotina que
encontrou cinco dos seis tetos mais antigos com folga entre 1,46× e 1,84×, e não
os 2× que o comentário deles prometia: o prompt de sistema tinha crescido ao
longo de semanas e os tetos ficaram parados. Nenhum tinha estourado — o modo de
falha de um teto largo demais é NÃO ACUSAR uma regressão de custo, e isso não
faz barulho, some.
...
Os 16 têm teto medido, todos recalibrados em 2026-09-07 contra o mesmo commit
do `sofia-bot` e o mesmo modelo. A divisão em levas abaixo é histórica — diz de
onde cada cenário veio, não em que estado ele está.
...
### Leva 2 — os dez restantes

Usam o mesmo vocabulário fechado, mais as duas chaves que nasceram nesta leva
(`respostas_assistente_max`, `agendamento_status`) e a `humano_pendente`, que
veio com a delegação. Todos com teto medido.

A calibração desta leva esteve **bloqueada** por um tempo, e a razão vale como
método: uma guarda faltava no `sofia-bot` (`completion.choices[0]`, em
`handleUserMessage`), e passada que morre em exceção não mede nada — calibrar
contra um bug de infraestrutura gastaria token medindo o número errado. Só
depois que a guarda entrou é que os tetos foram fixados.
...
| `delegacao-fora-da-grade` | pedido que cai num caso que a clínica não agenda faz a Sofia chamar a ferramenta de atendente humano — aferido por `contatos_estado.humano_pendente_desde`, nunca pelo texto |
| `marca-de-autoria-do-dono` | a fala do dono pelo app não vira promessa da Sofia: ela não converte oferta de terceiro em agendamento. Nasceu VERMELHO de propósito, antes da correção, e ficou verde quando ela entrou |
